...
saveDensityEst <- function(X, l, outData, nRun, filePath, stable, monitor){
  ...
  filePath <- gsub("densFun", "trainingPars/train3_densFun", filePath)
...
getLikelihood <- function(nRun, bayes, ewmaDays, filePath, funs, testNormal,
  ...
  if(compFigure){
    fileSave <- paste0("writings/paper2/figures/figXX_fullPlot_run", nRun, 
                       ".png")
  ...
  return(print("Done. Please copy file to SEAL/data/myanmar/trainingPars/"))
}

filePath <- "data/dataMyanmar/densFun/fun.Rdata"
...
## note that bayes argument is only for when compFigure=TRUE, otherwise it
## doesn't affect the creation of the density functions
sapply(c(2:3), getLikelihood, bayes=TRUE, ewmaDays=TRUE, filePath, funs,
       testNormal, createFuns, compFigure)

################################################################################
## test code (not finished) to run normality test on landscape stable zEWMAs
## (on a daily basis because the vectors would be so large)
...
reg <- "appLowEver"
path <- "dissertation/data/myanmar/"
ewmaFolder <- paste0(path, reg, "/ewma")

f <- list.files(ewmaFolder)
f <- f[order(as.numeric(gsub("day", "", f)))]
nData <- nCol*nMatRow

sapply(f, function(fil){
  # these are out of order but that's ok; we're looking at the numbers in
  ## aggregate
  cellFiles <- list.files(file.path(ewmaFolder, fil))
  
  cellVals <- sapply(cellFiles, function(X){
    ## Read in data for particular chunk
    ffP <- file(file.path(ewmaFolder, fil, X), 'rb')
    dat <- readBin(ffP, 'integer', n=nData)
    close(ffP)
    
    return(dat)
  })
  
  cellVals <- unlist(cellVals)/10000
  
})

# nRun=1, cell 75480, 75481 = ewma values of <-100