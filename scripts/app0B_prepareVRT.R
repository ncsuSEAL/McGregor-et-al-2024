...
##----------------------------------------------------------------------------##
# Step 1. Extract downloaded L8 images
...
################################################################################
# Step 2. Crop tifs and create the VRTs (AFTER making txt files)
library(terra)
library(data.table)
...
source("dissertation/scripts/funs/appA_createVRTs.R")
origVars <- ls()
s2Bands <- c("NDVI", "SCL")
epsg="32646"
...
################################################################################
# Step 2 archive. Prepare for creating VRTs used in landscape application

# Create txt files for S2 tifs. This is because S2 native files are a different
## tif for every band. Using these txt files, we will make a VRT combining them,
## and then using gdal make single raster crops containing the bands we want.
setwd("Z:/IanMcGregor/")
hpc <- FALSE
appRegion <- 1 #not important but needed for argsApp
applicationStep <- 3
source("dissertation/scripts/argsApp.R", local=TRUE)
source("dissertation/scripts/funs/appA_createVRTs.R")

sensor=c("sentinel2sr")

sapply(1:5, function(exts){
  ...
  sapply(sensor, prepareVRTtext, appLoc, s2tile, index="ndvi", runPath="")
})
